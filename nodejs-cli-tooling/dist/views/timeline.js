import { isEvent, isStateView, isActor, isCommand, } from '../models/types.js';
import { colors, formatElement, getElementStyle, rule } from './colors.js';
/**
 * Render the timeline view
 */
export function renderTimeline(model, showExamples = false) {
 ...
 */
export function renderHeader(model, viewName) {
    console.log();
    // Subtitle with model name, version and view mode
    const subtitle = [];
    subtitle.push(colors.cyan(model.name));
    if (model.version)
        subtitle.push(`v${model.version}`);
    if (viewName)
        subtitle.push(viewName);
 ...
 */
function renderTimelineElement(element, isLast, extraLines, showExamples) {
