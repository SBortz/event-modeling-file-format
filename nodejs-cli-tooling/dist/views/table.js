...
export function renderTable(model) {
    ...
    const stateViews = model.timeline.filter(isStateView);
 ...
 */
function renderDataFlowTree(model, events, stateViews, commands, actors) {
    ...
    const commandEventData = new Map();
    for (const cmd of commands) {
        const cmdKey = `${cmd.name}-${cmd.tick}`;
        const producedEvents = events
            .filter(e => e.producedBy === cmdKey)
