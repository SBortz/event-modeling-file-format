import { isEvent, isStateView, isCommand, isActor, } from '../models/types.js';
 ...
 */
export function renderSlice(model) {
    ...
    const eventsByCommandTick = new Map();
 ...
 */
function renderSlicePanel(slice, events, actors, eventsByCommandTick, isLast, slices, index) {
    ...
    if (isStateView(slice)) {
        ...
        if (readingActors.length > 0) {
            ...
            for (const actor of readingActors) {
                content.push(`  ${colors.actor('○')} ${colors.actor(actor.name)} ${colors.dim(`@${actor.tick}`)} → ${colors.command(actor.sendsCommand)}`);
            }
        }
    }
        ...
        if (triggeringActors.length > 0) {
            ...
            for (const actor of triggeringActors) {
                content.push(`  ${colors.actor('○')} ${colors.actor(actor.name)} ${colors.dim(`@${actor.tick}`)} ← ${colors.state(actor.readsView)}`);
            }
        }
